...
from PyQt5.QtWidgets import (QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                             ...
                             QTableWidget, QTableWidgetItem, QComboBox, QScrollArea, QGridLayout, QListWidget)
...
class SkiApp(QMainWindow):
    ...
    def add_date_fields_to_lewa_strona(self, frame):
        ...
        self.test_button_enabled = False  # Ustaw na False żeby wyłączyć
    ...
    def add_poziom_fields_to_srodek(self, frame):
        ...
        # Połączenie dla automatycznego wyszukiwania po Enter
        self.plec_pole.returnPressed.connect(self.znajdz_i_wyswietl)
    ...
    def add_preferencje_fields_to_prawa_strona(self, frame):
        ...
        self.wszystkie_radio.setChecked(True)  # Domyślnie zaznaczony
        ...
        # Połączenia dla automatycznego wyszukiwania po kliknięciu preferencji
        self.wszystkie_radio.clicked.connect(self.znajdz_i_wyswietl)
        self.slalom_radio.clicked.connect(self.znajdz_i_wyswietl)
        self.poza_trase_radio.clicked.connect(self.znajdz_i_wyswietl)
        self.caly_dzien_radio.clicked.connect(self.znajdz_i_wyswietl)
        self.gigant_radio.clicked.connect(self.znajdz_i_wyswietl)
        self.pomiedzy_radio.clicked.connect(self.znajdz_i_wyswietl)
    ...
    def pokaz_wszystkie_narty(self):
        ...
        try:
            ...
            dialog.setModal(True)
            dialog.resize(1400, 900)  # Większe okno
            dialog.setMinimumSize(1200, 700)  # Minimalny rozmiar
            ...
            filters_frame.setMaximumHeight(120)  # Ogranicz wysokość panelu filtrów
            filters_layout = QHBoxLayout(filters_frame)
            filters_layout.setSpacing(15)  # Odstępy między elementami
            ...
            search_input.setFixedHeight(25)
            search_input.textChanged.connect(lambda: self.filter_table(table, search_input.text(), self.get_selected_marki(marka_list), self.get_selected_poziomy(poziom_list), self.get_selected_plec(plec_list), self.get_selected_przeznaczenia(przeznaczenie_list)))
            search_layout.addWidget(search_label)
            search_layout.addWidget(search_input)
            
            # Filtr po marce - lista z wielokrotnym wyborem
            marka_container = QFrame()
            marka_layout = QVBoxLayout(marka_container)
            marka_layout.setContentsMargins(0, 0, 0, 0)
            marka_label = QLabel("Marka:")
            marka_label.setStyleSheet("font-weight: bold; color: #333;")
            marka_list = QListWidget()
            marka_list.setMaximumHeight(80)
            marka_list.setSelectionMode(QListWidget.MultiSelection)
            marki = sorted(list(set(narta.get('MARKA', '') for narta in narty if narta.get('MARKA'))))
            marka_list.addItems(marki)
            marka_list.itemSelectionChanged.connect(lambda: self.filter_table(table, search_input.text(), self.get_selected_marki(marka_list), self.get_selected_poziomy(poziom_list), self.get_selected_plec(plec_list), self.get_selected_przeznaczenia(przeznaczenie_list)))
            marka_layout.addWidget(marka_label)
            marka_layout.addWidget(marka_list)
            
            # Filtr po poziomie - lista z wielokrotnym wyborem
            poziom_container = QFrame()
            poziom_layout = QVBoxLayout(poziom_container)
            poziom_layout.setContentsMargins(0, 0, 0, 0)
            poziom_label = QLabel("Poziom:")
            poziom_label.setStyleSheet("font-weight: bold; color: #333;")
            poziom_list = QListWidget()
            poziom_list.setMaximumHeight(80)
            poziom_list.setSelectionMode(QListWidget.MultiSelection)
            poziomy = sorted(list(set(narta.get('POZIOM', '') for narta in narty if narta.get('POZIOM'))))
            poziom_list.addItems(poziomy)
            poziom_list.itemSelectionChanged.connect(lambda: self.filter_table(table, search_input.text(), self.get_selected_marki(marka_list), self.get_selected_poziomy(poziom_list), self.get_selected_plec(plec_list), self.get_selected_przeznaczenia(przeznaczenie_list)))
            poziom_layout.addWidget(poziom_label)
            poziom_layout.addWidget(poziom_list)
            
            # Filtr po płci - lista z wielokrotnym wyborem
            plec_container = QFrame()
            plec_layout = QVBoxLayout(plec_container)
            plec_layout.setContentsMargins(0, 0, 0, 0)
            plec_label = QLabel("Płeć:")
            plec_label.setStyleSheet("font-weight: bold; color: #333;")
            plec_list = QListWidget()
            plec_list.setMaximumHeight(80)
            plec_list.setSelectionMode(QListWidget.MultiSelection)
            plec_list.addItems(["M", "K", "U"])
            plec_list.itemSelectionChanged.connect(lambda: self.filter_table(table, search_input.text(), self.get_selected_marki(marka_list), self.get_selected_poziomy(poziom_list), self.get_selected_plec(plec_list), self.get_selected_przeznaczenia(przeznaczenie_list)))
            plec_layout.addWidget(plec_label)
            plec_layout.addWidget(plec_list)
            
            # Filtr po przeznaczeniu - lista z wielokrotnym wyborem
            przeznaczenie_container = QFrame()
            przeznaczenie_layout = QVBoxLayout(przeznaczenie_container)
            przeznaczenie_layout.setContentsMargins(0, 0, 0, 0)
            przeznaczenie_label = QLabel("Przeznaczenie:")
            przeznaczenie_label.setStyleSheet("font-weight: bold; color: #333;")
            przeznaczenie_list = QListWidget()
            przeznaczenie_list.setMaximumHeight(80)
            przeznaczenie_list.setSelectionMode(QListWidget.MultiSelection)
            przeznaczenia = sorted(list(set(narta.get('PRZEZNACZENIE', '') for narta in narty if narta.get('PRZEZNACZENIE'))))
            przeznaczenie_list.addItems(przeznaczenia)
            przeznaczenie_list.itemSelectionChanged.connect(lambda: self.filter_table(table, search_input.text(), self.get_selected_marki(marka_list), self.get_selected_poziomy(poziom_list), self.get_selected_plec(plec_list), self.get_selected_przeznaczenia(przeznaczenie_list)))
            przeznaczenie_layout.addWidget(przeznaczenie_label)
            przeznaczenie_layout.addWidget(przeznaczenie_list)
            
            # Przycisk wyczyść filtry
            clear_button = QPushButton("Wyczyść filtry")
            clear_button.setFixedHeight(30)
            clear_button.clicked.connect(lambda: self.clear_filters(search_input, marka_list, poziom_list, plec_list, przeznaczenie_list, table))
            
            # Dodaj elementy do layoutu filtrów
            filters_layout.addWidget(search_container)
            filters_layout.addWidget(marka_container)
            filters_layout.addWidget(poziom_container)
            filters_layout.addWidget(plec_container)
            filters_layout.addWidget(przeznaczenie_container)
            ...
            table.setColumnCount(14)  # Poprawiona liczba kolumn
            ...
            # Ustaw tabelę żeby zajmowała resztę okna
            ...
            # Dodaj tabelę do layoutu z priorytetem rozszerzania
            layout.addWidget(table, 1)  # Współczynnik 1 = rozszerza się
            
            # ========== PRZYCISKI ==========
            buttons_frame = QFrame()
            buttons_frame.setMaximumHeight(50)  # Ogranicz wysokość przycisków
            ...
            layout.addWidget(buttons_frame, 0)  # Współczynnik 0 = nie rozszerza się
    ...
    def get_selected_marki(self, marka_list):
        """Pobiera wybrane marki z listy"""
        selected_items = marka_list.selectedItems()
        return [item.text() for item in selected_items]
    
    def get_selected_poziomy(self, poziom_list):
        """Pobiera wybrane poziomy z listy"""
        selected_items = poziom_list.selectedItems()
        return [item.text() for item in selected_items]
    
    def get_selected_plec(self, plec_list):
        """Pobiera wybrane płeć z listy"""
        selected_items = plec_list.selectedItems()
        return [item.text() for item in selected_items]
    
    def get_selected_przeznaczenia(self, przeznaczenie_list):
        """Pobiera wybrane przeznaczenia z listy"""
        selected_items = przeznaczenie_list.selectedItems()
        return [item.text() for item in selected_items]
    
    def filter_table(self, table, search_text, marka_filters, poziom_filters, plec_filters, przeznaczenie_filters):
        ...
        try:
            ...
            for narta in table.original_data:
                ...
                # Sprawdź filtr marki (wielokrotny wybór)
                if marka_filters:  # Jeśli są wybrane marki
                    if narta.get('MARKA', '') not in marka_filters:
                        continue
                
                # Sprawdź filtr poziomu (wielokrotny wybór)
                if poziom_filters:  # Jeśli są wybrane poziomy
                    if narta.get('POZIOM', '') not in poziom_filters:
                        continue
                
                # Sprawdź filtr płci (wielokrotny wybór)
                if plec_filters:  # Jeśli są wybrane płeć
                    if narta.get('PLEC', '').strip() not in plec_filters:
                        continue
                
                # Sprawdź filtr przeznaczenia (wielokrotny wybór)
                if przeznaczenie_filters:  # Jeśli są wybrane przeznaczenia
                    przeznaczenie = narta.get('PRZEZNACZENIE', '')
                    # Sprawdź czy narta ma którekolwiek z wybranych przeznaczeń
                    has_matching_przeznaczenie = False
                    for selected_przeznaczenie in przeznaczenie_filters:
                        if selected_przeznaczenie in przeznaczenie:
                            has_matching_przeznaczenie = True
                            break
                    if not has_matching_przeznaczenie:
                        continue
    ...
    def clear_filters(self, search_input, marka_list, poziom_list, plec_list, przeznaczenie_list, table):
        """Czyści wszystkie filtry"""
        try:
            search_input.clear()
            marka_list.clearSelection()
            poziom_list.clearSelection()
            plec_list.clearSelection()
            przeznaczenie_list.clearSelection()
